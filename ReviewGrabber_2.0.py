...
for asin in ASIN_list:
    ...
    while page <= iterator:
        ...
        with open(asin + 'metadata.csv', 'a') as fa:
            if counter == 0:
                fa.write("Date," + "Stars," + "Helpful Votes\n")
                counter += 1
            for rev in soup_1.findAll('div', {'data-hook': "review", 'class': "a-section review"}):
                crazy = str(rev)
                sub_soup = BeautifulSoup(crazy, "lxml")
                date = sub_soup.find('span', {'data-hook': "review-date"}).string
                date = date.replace('on ', '')
                date = date.replace(', ', '/')
                date = date.replace(' ', '/')

                stars = str(sub_soup.find('span', {'class': "a-icon-alt"}).string[0:3])
                try:
                    helpful_votes = sub_soup.find('span', {'class': "a-size-base a-color-tertiary cr-vote-text",'data-hook': "helpful-vote-statement"})
